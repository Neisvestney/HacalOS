...
#define HBA_PxCMD_FR 0x4000
    ...
    void Port::Configure() {
        ...
        void *newBase = globalPageFrameAllocator.RequestPage();
        ...
        void *fisBase = globalPageFrameAllocator.RequestPage();
        hbaPort->fisBaseAddress = (uint32_t) (uint64_t) fisBase;
        hbaPort->fisBaseAddressUpper = (uint32_t) ((uint64_t) fisBase >> 32);
        memset(fisBase, 0, 256);

        HBACommandHeader *cmdHeader = (HBACommandHeader *) ((uint64_t) hbaPort->commandListBase +
                                                            ((uint64_t) hbaPort->commandListBaseUpper << 32));

        for (int i = 0; i < 32; i++) {
            cmdHeader[i].prdtLength = 8;

            void *cmdTableAddress = globalPageFrameAllocator.RequestPage();
    ...
    AHCIDriver::AHCIDriver(PCI::PCIDeviceHeader *pciBaseAddress) {
        ...
        this->abar = (HBAMemory *) (uint64_t) ((PCI::PCIDeviceHeader0 *) pciBaseAddress)->bar5;
        kernelPageTableManager.MapMemory(abar, abar);
        ProbePorts();
        ...
        for (int i = 0; i < portCount; i++) {
            ...
            port->buffer = (uint8_t *) globalPageFrameAllocator.RequestPage();
            memset(port->buffer, 0, 0x1000);

            port->Read(0, 1, port->buffer);
            for (int t = 0; t < 512; t++) {
                basicRenderer.PutChar(port->buffer[t]);
            }
